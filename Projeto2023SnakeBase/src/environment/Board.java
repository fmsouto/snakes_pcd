...
public abstract class Board extends Observable {
	...
	public Boolean checkIfCellExists(BoardPosition cellCoord){
		if(cellCoord.x >= 0 || cellCoord.x < NUM_ROWS || cellCoord.y >= 0 || cellCoord.y < NUM_COLUMNS)
			return true;
		else
			return false;
	}
	...
	public void setGoalPosition(BoardPosition goalPosition) {
		...
		while(true){
			if(!cells[goalPosition.x][goalPosition.y].isOcupied()){
			System.err.println("POSICAO GOAL NOVO: " + "X: " + goalPosition.x + " Y: " + goalPosition.y);
			cells[goalPosition.x][goalPosition.y].setGameElement(g);
...
//					System.out.println("Object placed at: "+pos);					
				}
				placed=true;
			}
		}
